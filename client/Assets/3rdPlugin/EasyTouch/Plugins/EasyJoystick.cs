...
public class EasyJoystick : MonoBehaviour {
				...
				if (!dynamicJoystick){
					...
					if (!virtualJoystick){
						...
						if (virtualJoystick){
                            ...
                            //if (gesture.position.x > Screen.width - 50)
                            //{
                            //    gesture.position.x = Screen.width - 50;
                            //}

                            //if (gesture.position.y < 50 )
                            //{
                            //    gesture.position.y = 50 ;
                            //}

                            //if (gesture.position.y > Screen.height - 50 )
                            //{
                            //    gesture.position.y = Screen.height - 50 ;
                            //}
                            gesture.position = F_LimitVectorRange(gesture.position, new Vector2(50, 50),
    new Vector2(Screen.width - 50, Screen.height - 50));
			...
			if (isActivated){
				...
				if (gesture.fingerIndex == joystickIndex){
                    ...
                    if (virtualJoystick)
                    {
                        ...
                        Vector2 tesPos = gesture.position;

                        Vector2 joyticPos = new Vector2(joystickCenter.x * VirtualScreen.xRatio, (VirtualScreen.height - joystickCenter.y) * VirtualScreen.yRatio);

                        if (Vector2.Distance(tesPos, joyticPos) > mAreaRadiu)
                        {
                            Vector2 normoal = (joyticPos - tesPos).normalized;

                            joyticPos = tesPos + normoal * mAreaRadiu;

                            Vector2 centPos = joyticPos;
                            ...
                            //if (centPos.x > Screen.width - 50f)
                            //{
                            //    centPos.x = Screen.width - 50f;
                            //}
                            ...
                            //if (centPos.y > Screen.height - 50)
                            //{
                            //    centPos.y = Screen.height - 50;
                            //}

                            centPos = F_LimitVectorRange(centPos, new Vector2(50, 50),
                                new Vector2(Screen.width - 50, Screen.height - 50));

                            joystickCenter = new Vector2(centPos.x / VirtualScreen.xRatio, VirtualScreen.height - centPos.y / VirtualScreen.yRatio);
                            JoyAnchor = JoystickAnchor.None;
                            joystickIndex = gesture.fingerIndex;
                        }
    ...
    private Vector2 F_LimitVectorRange(Vector2 pPos,Vector2 pMinVect, Vector2 pMaxVect)
    {

        if (pPos.x < pMinVect.x)
        {
            pPos.x = pMinVect.x;
        }

        if (pPos.x > pMaxVect.x)
        {
            pPos.x = pMaxVect.x;
        }

        if (pPos.y < pMinVect.y)
        {
            pPos.y = pMinVect.y;
        }

        if (pPos.y > pMaxVect.y)
        {
            pPos.y = pMaxVect.y;
        }
        return pPos;
